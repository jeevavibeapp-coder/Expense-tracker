"""Deterministic SMS / UPI transaction parser (no external AI dependency).

Extracts amount, direction, merchant/counterparty, reference number and
date/time from common Indian bank & UPI SMS formats using ordered regexes.
"""
...
def _parse_merchant(text: str) -> Optional[str]:
    ...
    # Strip a VPA suffix (name@bank) down to the handle.
...
def parse_sms(text: str) -> ParsedSMS:
    ...
    if _CREDIT_RE.search(text) and not _DEBIT_RE.search(text):
        result.type = "income"
    else:
        result.type = "expense"
