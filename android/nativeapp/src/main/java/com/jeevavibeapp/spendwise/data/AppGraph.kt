    ...
    fun database(context: Context): SpendDatabase =
        db ?: synchronized(this) {
            db ?: Room.databaseBuilder(
                ...
                // Declaring a migration is not enough — Room only runs the
                // ones it is handed, and refuses to open a database it has no
                // path for. Without this line a v1 install crashes on first
